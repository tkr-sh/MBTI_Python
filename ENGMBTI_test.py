from ENGquestions_mbti import question_about_mbti as FC # FC <=> Fonctions Cognitives 
import random
...
def analysis(TI,TE,NI,NE,SI,SE,FI,FE):
    ...
    def calcul_of_points(dom, aux, ter, inf, MAX, MIN):
        ...
        # Calculs with Dom as a reference
        """
        ...
        """
        # Calculs with Max as a reference
    ...
    if ordonned_list[1]/ordonned_list[0]>0.90:
        print(f"Results between{ordonned_list_str[0]} and {ordonned_list_str[1]} are really close.\nHowever, you show a slight preference for {ordonned_list_str[0]}\n{ordonned_list_str[2]} is also possible.\nMore details on the list above.")
    elif ordonned_list[1]/ordonned_list[0]<0.80:
        print(f"With near certainty you are {ordonned_list_str[0]}.\nMore details on the list above.")
    else:
        print(f"We think you are {ordonned_list_str[0]}.\nHowever, {ordonned_list_str[1]} is also possible, but less likely.\n{ordonned_list_str[2]} is also possible.\nMore details on the list above.")
...
def beginning():
    code_or_test = True
    while code_or_test:
        code = input("Do you want to take a test [T] or enter your code [C]??\n>>>")
        if code.upper() not in ["T", 'C']:
            print("I did not understand. Enter C for the code and T for the test.")
        elif code == "C":
            code_nb = input("Enter the code:\n")
...
def questions_test(lettre, fonction_intro, fonction_extra, diff):
    for i in range(0,10):
        ...
        while waiting:
            waiting=not waiting
            réponse = input("Your answer >").replace("[","").replace("]","")
            if len(réponse)>2:
                réponse = réponse[:3]
            if réponse[0] == '-':
...
print("Your code is:", code,"\nKeep it if you want to review your results")               #
###########################################################################################
